...
export default function Activities({ activities }: ActivitiesProps) {
  ...
  const hasTitle = !!(activities?.sectionTitle && activities.sectionTitle.trim() !== '');
  const hasDescription = !!(activities?.sectionDescription && activities.sectionDescription.trim() !== '');
  const hasDevelopment = Array.isArray(activities?.development) && activities.development.length > 0;

  if (!hasTitle && !hasDescription && !hasDevelopment) return null;
